/**
 * Golden contract tests for Mauritius Law MCP.
 *
 * Boots a real MCP server in-process (via InMemoryTransport) and verifies
 * tool outputs against the golden-tests.json fixture file.
 *
 * Network-dependent assertions (upstream_text_hash, citation_resolves)
 * are gated behind CONTRACT_MODE=nightly so that normal `npm test` never
...
describe(`Contract tests: ${fixture.mcp_name}`, () => {
  beforeAll(async () => {
    const dbPath =
      process.env['MU_LAW_DB_PATH'] ?? join(__dirname, '..', '..', 'data', 'database.db');
    ...
    const server = new Server(
      { name: 'mauritian-law-test', version: '0.0.0' },
      { capabilities: { tools: {} } },
    );
