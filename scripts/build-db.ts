#!/usr/bin/env tsx
/**
 * Database builder for Mauritius Law MCP server.
 *
...
CREATE TABLE legal_documents (
  ...
  status TEXT NOT NULL DEFAULT 'in_force'
    CHECK(status IN ('in_force', 'amended', 'repealed', 'partially_suspended', 'not_yet_in_force')),
  issued_date TEXT,
  in_force_date TEXT,
  url TEXT,
  description TEXT,
...
-- EU/International Documents (directives and regulations)
...
-- EU/International References (links national provisions to EU documents)
...
function extractEuReferences(text: string): ExtractedEUReference[] {
  ...
  for (const pattern of patterns) {
    ...
    while ((match = pattern.exec(text)) !== null) {
      ...
      const referenceType: EUReferenceType = /\b(implement|align|transpos|equivalent|model)/i.test(referenceContext) ? 'implements' : 'references';
...
function buildDatabase(): void {
  console.log('Building Mauritius Law MCP database...\n');
          ...
          if (extractedRefs.length > 0) {
            ...
            for (const ref of extractedRefs) {
              ...
              const euInsert = insertEuDocument.run(
                ...
                shortName, shortName, eurLexUrl, 'Auto-extracted from Mauritian statute text',
              );
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('source', 'lawmauritian.com');
