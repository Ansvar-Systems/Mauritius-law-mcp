#!/usr/bin/env tsx
/**
 * Mauritian Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'http://lawmauritian.com';
const PORTAL_NAME = 'Mauritius Law';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/mauritian-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Mauritian Law MCP — Data Freshness Check');
