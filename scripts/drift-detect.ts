#!/usr/bin/env tsx
/**
 * Drift detection for Mauritius Law MCP.
 *
 * Checks if upstream Mauritius Law content has changed since last ingestion.
...
async function main(): Promise<void> {
  console.log('Mauritius Law MCP — Drift Detection');
  console.log('================================\n');
  ...
  for (const hash of fixture.provisions) {
    ...
    try {
      const response = await fetch(hash.upstream_url, {
        headers: { 'User-Agent': 'mauritian-law-mcp/1.0 drift-detect' },
      });
