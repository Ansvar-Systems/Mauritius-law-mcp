#!/usr/bin/env tsx
/**
 * Mauritius Law MCP -- Census-Driven Ingestion Pipeline (PDF-based)
 *
 * Reads data/census.json and fetches + parses every ingestable Act
 * from lawmauritian.com. Mauritian proclamations are published as PDFs.
 *
 * Pipeline per act:
 ...
 *
 * Data source: lawmauritian.com (Federal Negarit Gazette)
...
async function downloadPdf(url: string, outputPath: string): Promise<{ status: number; size: number }> {
  ...
  const response = await fetch(url, {
    headers: {
      'User-Agent': 'mauritian-law-mcp/1.0 (https://github.com/Ansvar-Systems/mauritian-law-mcp; hello@ansvar.ai)',
    },
    redirect: 'follow',
  });
...
async function main(): Promise<void> {
  ...
  console.log('Mauritius Law MCP -- Ingestion Pipeline (PDF-based)');
  console.log('===================================================\n');
  console.log(`  Source: lawmauritian.com (Federal Negarit Gazette)`);
  ...
  console.log(`\n  Source:      lawmauritian.com (Federal Negarit Gazette PDFs)`);
