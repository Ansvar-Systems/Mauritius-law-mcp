/**
 * get_mauritian_implementations — Find Mauritian statutes implementing a specific EU directive/regulation
...
export interface GetMauritianImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface MauritianImplementationResult {
  document_id: string;
...
export async function getMauritianImplementations(
  db: InstanceType<typeof Database>,
  input: GetMauritianImplementationsInput,
): Promise<ToolResponse<MauritianImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as MauritianImplementationResult[];
