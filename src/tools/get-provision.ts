/**
 * get_provision — Retrieve specific provision(s) from a Mauritian statute.
 */
...
export async function getProvision(
  ...
  if (ref) {
    ...
    // Try with "s" prefix (e.g., "1" -> "s1") — Mauritian "Section" convention
    if (!provision) {
  ...
  return {
    ...
    _citation: buildProvisionCitation(
      resolvedId,
      docRow.title,
      String(provisions[0]?.provision_ref || ''),
      input.document_id,
      input.section || input.provision_ref || '',
      docRow.url || null,
      null,
