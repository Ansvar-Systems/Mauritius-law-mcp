/**
 * search_eu_implementations — Search EU directives/regulations that have Mauritian implementing legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_mauritian_implementation?: boolean;
  limit?: number;
}
...
export interface EUImplementationSearchResult {
  ...
  mauritian_statute_count: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as mauritian_statute_count
  ...
  if (input.has_mauritian_implementation) {
    sql += ' HAVING mauritian_statute_count > 0';
  }
